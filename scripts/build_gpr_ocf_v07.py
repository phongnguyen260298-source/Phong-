...
def replace_text(s):
 ...
 return s
# Remove stale WCB provenance labels globally while retaining historical sheet names.
for ws in wb.worksheets:
 if ws.title in ('24_QA_V04',): continue
 for row in ws.iter_rows():
  for c in row:
   if isinstance(c.value,str): c.value=replace_text(c.value)
# Final lock: patch H1/H2 WCB rows, numeric value(s) and interpretation text.
ws=wb['09_FINAL_LOCK']
for key,val in [('H1_WCB',.0293),('H2_WCB',.0018)]:
 rows=find_rows(ws,key)
 assert rows, f'{key} row not found'
 for r in rows:
  for c in ws[r]:
   if isinstance(c.value,(int,float)) and abs(float(c.value)-(.0308 if key=='H1_WCB' else .0023))<1e-9: c.value=val
   elif isinstance(c.value,str):
    c.value=c.value.replace('0.0308','0.0293').replace('0,0308','0,0293').replace('0.0023','0.0018').replace('0,0023','0,0018')
# Number map: update main WCB values and v06 provenance.
ws=wb['14_NUMBER_MAP']
for row in ws.iter_rows():
 ...
 if 'wcb' in txt:
  for c in row:
   if isinstance(c.value,(int,float)):
    ...
    c.value=c.value.replace('0.0308','0.0293').replace('0,0308','0,0293').replace('0.0023','0.0018').replace('0,0023','0,0018'); c.value=replace_text(c.value)
# Robustness: preserve corrected p-values and make df notes explicit.
...
for label,fallback in [('Lead 1',11),('Lag 1',12),('Lag 2',13),('Exclude 2022Q1-Q2',14)]:
 ...
 note=ws.cell(r,max(ws.max_column,11)); note.value=f'dynamic inference uses Student-t with df=Gq-1=t({df}); v06/v07 synchronized p-values from locked beta/SE'
# Mark old QA as historical only.
...
# Ensure stale main WCB values/provenance are absent outside historical v04 QA.
stale=[]; errors=[]
