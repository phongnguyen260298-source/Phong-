...
def fetch_one(sym):
    ...
    for k in range(4):
        try:
            ...
            if d is None or d.empty: return sym,None,'EMPTY'
            d=d.copy()
            ...
            vc=next((c for c in ['volume','Volume','match_volume','total_volume'] if c in d.columns),None)
            if tc is None or cc is None: return sym,None,f'SCHEMA:{list(d.columns)}'
            z=pd.DataFrame({'symbol':sym,'date':pd.to_datetime(d[tc],errors='coerce'),'close':pd.to_numeric(d[cc],errors='coerce'),'volume':pd.to_numeric(d[vc],errors='coerce') if vc else np.nan})
            ...
            last=e; time.sleep(min(8,1.2*(k+1)))
    return sym,None,f'{type(last).__name__}:{last}'

frames=[]; errors=[]
with ThreadPoolExecutor(max_workers=2) as pool:
    fs={pool.submit(fetch_one,s):s for s in SYMBOLS}
    for i,f in enumerate(as_completed(fs),1):
        sym,z,e=f.result()
        if z is not None: frames.append(z)
        else: errors.append({'symbol':sym,'error':e})
        if i%50==0: print('KBS',i,'/',len(SYMBOLS),'ok',len(frames),'err',len(errors),flush=True)
        time.sleep(.05)
pd.DataFrame(errors).to_csv(OUT/'price_errors_kbs.csv',index=False)
if not frames: raise RuntimeError('No KBS price data')
px=pd.concat(frames,ignore_index=True).sort_values(['symbol','date'])
px['ret']=px.groupby('symbol')['close'].pct_change()
mkt=px.groupby('date')['ret'].median().rename('mkt_ret'); px=px.merge(mkt,on='date',how='left'); px['ar']=px.ret-px.mkt_ret
...
r=requests.get(url,timeout=90,headers={'User-Agent':'Mozilla/5.0 academic-research/1.0'}); r.raise_for_status(); (OUT/'ai_gpr_data_daily.csv').write_bytes(r.content)
...
for c in SHOCKS:
    g[c]=pd.to_numeric(g[c],errors='coerce'); g[c+'_innov']=np.log1p(g[c].clip(lower=0)).diff()
trading=pd.DatetimeIndex(sorted(px.date.dropna().unique())); mrows=[]
for _,rr in g.dropna(subset=['date']).iterrows():
    j=trading.searchsorted(rr.date)
    if j>=len(trading):continue
    row={'trade_date':trading[j]}
    for c in SHOCKS:row[c+'_innov']=rr[c+'_innov'];row[c+'_level']=rr[c]
    mrows.append(row)
mg=pd.DataFrame(mrows); agg={c+'_innov':'sum' for c in SHOCKS}|{c+'_level':'last' for c in SHOCKS}; mg=mg.groupby('trade_date',as_index=False).agg(agg).sort_values('trade_date')
for c in SHOCKS:
...
for grp in GROUPS:
    ...
    if 0 in tmp.columns and 1 in tmp.columns:
        s=(tmp[1]-tmp[0]).rename('spread').reset_index();s['group']=grp;parts.append(s)
...
for grp in GROUPS:
    ...
    for shock in [c+'_z' for c in SHOCKS]:
        ...
        if len(z)<60 or z[shock].std(ddof=0)==0:regrows.append({'group':grp,'shock':shock,'beta':np.nan,'p':np.nan,'n':len(z)});continue
...
by={s:z.set_index('date').sort_index() for s,z in p.groupby('symbol')}; gl=G.set_index('symbol')[GROUPS]; rows=[]
for ed in events:
    idx=trading.get_indexer([ed])[0]
    if idx<0:continue
    prewin=trading[max(0,idx-5):idx]
    for sym,zs in by.items():
        row={'symbol':sym,'event_trade_date':ed};valid=False
        for h in [1,5,20]:
            win=trading[idx:min(len(trading),idx+h)]; car=zs.reindex(win).ar.sum(min_count=1);row[f'car_{h}']=car;valid=valid or np.isfinite(car)
        row['pre_car_5']=zs.reindex(prewin).ar.sum(min_count=1) if len(prewin) else np.nan
        win5=trading[idx:min(len(trading),idx+5)]; av=zs.reindex(win5).abn_logvol;row['abn_volume_5']=av.mean() if av.notna().any() else np.nan
        row['belief_pressure_5']=max(0.,-row['car_5'])*max(0.,row['abn_volume_5']) if np.isfinite(row['car_5']) and np.isfinite(row['abn_volume_5']) else np.nan
        if sym in gl.index:
            for c in GROUPS:row[c]=int(gl.loc[sym,c])
        if valid:rows.append(row)
ef=pd.DataFrame(rows);ef.to_csv(OUT/'event_firm_metrics.csv',index=False)
...
for grp in GROUPS:
    for metric in ['pre_car_5','car_1','car_5','car_20','abn_volume_5','belief_pressure_5']:
        for ed,ze in ef.dropna(subset=[grp,metric]).groupby('event_trade_date'):
            ...
            evrows.append({'group':grp,'metric':metric,'event_trade_date':ed,'diff':diff,'p':pv,'n_t':len(a),'n_c':len(b)})
ev=pd.DataFrame(evrows);ev.to_csv(OUT/'event_group_metrics.csv',index=False)
pool=[]
for (grp,metric),z in ev.groupby(['group','metric']):
    pp=z.p.dropna().clip(1e-12,1);fisher=float(chi2.sf(-2*np.log(pp).sum(),2*len(pp))) if len(pp) else np.nan;pool.append({'group':grp,'metric':metric,'mean_diff':z['diff'].mean(),'median_diff':z['diff'].median(),'events':len(z),'share_negative':(z['diff']<0).mean(),'fisher_p':fisher})
...
for name,grp,shock in C:
    ...
    rr=rr.iloc[0]
    def get(metric,col):
        z=pool[(pool.group==grp)&(pool.metric==metric)];return float(z.iloc[0][col]) if len(z) else np.nan
...
summary={'price_source':'vnstock community KBS','symbols_target':len(SYMBOLS),'symbols_price_ok':px.symbol.nunique(),'price_rows':len(px),'events_n':len(events),'directions':rank.replace({np.nan:None}).to_dict(orient='records')};(OUT/'summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(summary,ensure_ascii=False,indent=2))
