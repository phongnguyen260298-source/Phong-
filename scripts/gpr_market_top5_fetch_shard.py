...
SHARD=int(os.getenv('SHARD','0')); N_SHARDS=int(os.getenv('N_SHARDS','10'))
PAUSE=float(os.getenv('REQUEST_PAUSE','4.2'))
OUT=Path('data/gpr_market_top5_v3/shards'); OUT.mkdir(parents=True, exist_ok=True)
...
for i,sym0 in enumerate(symbols,1):
    ...
    print(f'shard {SHARD} {i}/{len(symbols)} {sym} ok={z is not None}',flush=True)
    time.sleep(PAUSE)
