...
for qq,name in [(0.50,'high50'),(2/3,'high33'),(0.75,'high25')]:
    cut=d[['symbol','liab_exp']].drop_duplicates().liab_exp.quantile(qq); d[name]=(d.liab_exp>=cut).astype(int)
for s in ['Threat','Acts']:
    mp=qg.set_index('period_q')[s]; ordered=list(qg.period_q)
...
for s in ['Threat','Acts']:
    ...
    add('lag1',s,'liab_exp',[f'{s}_lag1*liab_exp'],key=f'{s}_lag1_X_liab_exp')
    add('lead1_placebo',s,'liab_exp',[f'{s}_lead1*liab_exp'],key=f'{s}_lead1_X_liab_exp')
for s in ['Threat','Acts']:
...
try:
    ...
    for s in ['Threat','Acts']:
        rows.append({'spec':'joint_threat_acts','shock':s,'exposure':'liab_exp','beta':np.nan,'se':np.nan,'p':np.nan,'error':f'{type(ex).__name__}: {ex}'})
