...
OUT=Path('data/gpr_market_top5_v3'); OUT.mkdir(parents=True, exist_ok=True)
SHARDS=OUT/'shards'
...
px['symbol']=px['symbol'].astype(str).str.upper(); px['date']=pd.to_datetime(px['date'],errors='coerce'); px['close']=pd.to_numeric(px['close'],errors='coerce')
px=px.dropna().drop_duplicates(['symbol','date']).sort_values(['symbol','date'])
...
coverage=[]
for grp in GROUPS:
    coverage.append({'group':grp,'firms_total':int(base.symbol.nunique()),'firms_price':int(p.symbol.nunique()),'treated_firms':int(base.loc[pd.to_numeric(base[grp],errors='coerce')==1,'symbol'].nunique())})
...
# Event dates: top Threat innovations, spaced >=10 calendar days.
...
for f in err_files:
    try: errn+=len(pd.read_csv(f))
    except: pass
summary={'price_shards':len(files),'symbols_target':int(base.symbol.nunique()),'symbols_price_ok':int(px.symbol.nunique()),'price_rows':int(len(px)),'errors':errn,'gpr_cols':{'ai':ai,'original':orig,'threats':thr,'acts':act},'top5':rank.replace({np.nan:None}).to_dict(orient='records')}
